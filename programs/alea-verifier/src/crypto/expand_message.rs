...
pub fn expand_message_xmd(msg: &[u8], dst: &[u8], len_in_bytes: usize) -> [u8; 96] {
    assert_eq!(len_in_bytes, 96);
    let s_in_bytes: usize = 136; // keccak256 rate (1088 bits)

    // DST_prime = DST || I2OSP(len(DST), 1)
    let dst_len_byte = dst.len() as u8;
    let mut dst_prime = Vec::with_capacity(dst.len() + 1);
    dst_prime.extend_from_slice(dst);
    dst_prime.push(dst_len_byte);

    let z_pad = [0u8; 136];
    assert_eq!(z_pad.len(), s_in_bytes);

    // l_i_b_str = I2OSP(len_in_bytes, 2) = 0x0060
    let l_i_b_str = (len_in_bytes as u16).to_be_bytes();

    let zero_byte = [0u8; 1];

    // b_0 = keccak256(Z_pad || msg || l_i_b_str || I2OSP(0, 1) || DST_prime)
    let b_0 = keccak::hashv(&[&z_pad, msg, &l_i_b_str, &zero_byte, &dst_prime]);

    // b_1 = keccak256(b_0 || I2OSP(1, 1) || DST_prime)
    let counter_1 = [1u8];
    let b_1 = keccak::hashv(&[b_0.as_ref(), &counter_1, &dst_prime]);

    // b_2 = keccak256(XOR(b_0, b_1) || I2OSP(2, 1) || DST_prime)
    let mut xor_buf = [0u8; 32];
    for i in 0..32 {
        xor_buf[i] = b_0.as_ref()[i] ^ b_1.as_ref()[i];
    }
    let counter_2 = [2u8];
    let b_2 = keccak::hashv(&[&xor_buf, &counter_2, &dst_prime]);

    // b_3 = keccak256(XOR(b_0, b_2) || I2OSP(3, 1) || DST_prime)
    for i in 0..32 {
        xor_buf[i] = b_0.as_ref()[i] ^ b_2.as_ref()[i];
    }
    let counter_3 = [3u8];
    let b_3 = keccak::hashv(&[&xor_buf, &counter_3, &dst_prime]);
...
/// Each from 48 bytes (384 bits > 254 bits), reduced mod p for uniform distribution
pub fn hash_to_field(msg: &[u8]) -> (Fq, Fq) {
    let expanded = expand_message_xmd(msg, DST, 96);
...
mod tests {
    ...
    fn expand_message_round_1() {
        ...
        let result = expand_message_xmd(&msg_hash, DST, 96);
